...
Сопоставление имён регистронезависимо и устойчиво к русским склонениям (матч по
словам через длину общего префикса).
"""
...
def _fit_to_budget(client, model, slice_data, budget):
    """Режет срез с конца (наименее важные узлы) под бюджет токенов. Меряем полный
    блок (1 вызов); если влез — готово. Иначе прикидываем долю и доуточняем ≤4 шага."""
    ...
    def build():
        ...
        return render_slice({"nodes": nodes, "edges": edges}), edges
...
def retrieve_block(client, model: str, query_text: str,
    ...
    Логирует фактический размер инъекции — чтобы в логах было видно, сколько
    токенов память добавляет в контекст на каждом чтении.
    """
