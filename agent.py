...
- `delete_task(title)` — удалить задачу безвозвратно (только по явной просьбе «удали совсем»; иначе предпочитай archive_task).
...
MEMORY_TOOLS = [
    ...
    },
    {
        "name": "delete_task",
        "description": (
            "Полностью УДАЛИТЬ задачу (безвозвратно, не архив). Вызывай ТОЛЬКО по "
            "явной просьбе («удали задачу совсем»). Если задачу нужно просто убрать "
            "с глаз — предпочти archive_task."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
